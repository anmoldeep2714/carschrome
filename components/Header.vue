...
const onInput = () => {
  debounce(() => fetchSuggestions(search.value))
}
...
<template>
                ...
                <div class="inner">
                    ...
                    <div class="search-language-cart-wrapper">
                        <div class="search-bar-form-wrapper">
                            <div class="add-new-v-toggle" @click="vehiclePopup.openPopup(false)">
                                <span class="icon">
                                ...
                                <span class="text" v-if="vehiclePopup.storeVehicles.length == 0">All vehicles</span>
                                <span class="text" v-if="vehiclePopup.storeVehicles.length > 0">{{
                                    vehiclePopup.storeVehicles[0].year }} {{ vehiclePopup.storeVehicles[0].make.name }}
                                    {{ vehiclePopup.storeVehicles[0].model.name }}</span>
                            </div>
                            <form class="form">
                                <div class="input-wrapper">
                                    <input type="text" :placeholder="$t('menu.search_input_placeholder')"  v-model="search"
                                    @input="onInput">
                                    <div class="reset-toggle"></div>
                                </div>
                                <button type="submit">
                                    <span>{{ $t('menu.search_btn_title') }}</span>
                                </button>
                            </form>
                            <div class="search-bar-suggestions-wrapper" :class="{'active':showSuggestions}">
                                <ul v-if="showSuggestions && suggestions.length > 0">
                                    <li v-for="(item, index) in suggestions"
                                    :key="index">
                                        <a><span v-html="item.name"></span></a>
                                    </li>
                                </ul>
                                <div class="no-results" v-if="showSuggestions && !suggestions.length">No results found.</div>
                            </div>
                        </div>
    ...
    <VehicleSelector :reloadVehiclePop="false" :closeVehiclePop="true"></VehicleSelector>
</template>